    ...
    //FIND CHESSBOARDS AND CORNERS THEREIN:
        ...
        if( displayCorners )
        {
            ...
            cvDrawChessboardCorners( cimg, cvSize(nx, ny), &temp[0],
                count, result );
            IplImage* cimg1 = cvCreateImage(cvGetSize(img), IPL_DEPTH_8U, 3);
            cvResize(cimg, cimg1);
...
int 
StereoRectify(IplImage** s_img, IplImage** s_img2, CvMat** mx1, CvMat** my1, CvMat** mx2, CvMat**my2)
{
	CvFileStorage* fstorage = cvOpenFileStorage("intrinsics.yml", NULL, CV_STORAGE_READ);
	if (!fstorage) {printf("Não carregou intrinsics.yml\n"); return -1;}
	CvMat* _pM1 = (CvMat*)  cvReadByName( fstorage, 0, "M1" );
	CvMat* _pD1 = (CvMat*)  cvReadByName( fstorage, 0, "D1" );
	CvMat* _pM2 = (CvMat*)  cvReadByName( fstorage, 0, "M2" );
	CvMat* _pD2 = (CvMat*)  cvReadByName( fstorage, 0, "D2" );
	cvReleaseFileStorage(&fstorage);

//	cout << "\n\n\t\trows= " << _M1->rows << "\n\n\t\tcols= " << _M1->cols<<endl;

	fstorage = cvOpenFileStorage("extrinsics.yml", NULL, CV_STORAGE_READ);
	CvMat* _pR = (CvMat*)  cvReadByName( fstorage, 0, "R" );
	CvMat* _pT = (CvMat*)  cvReadByName( fstorage, 0, "T" );
	cvReleaseFileStorage(&fstorage);
	
	IplImage* image = cvCloneImage(*s_img); 
	IplImage* object = cvCloneImage(*s_img2);
	if (!image) { printf( "nao carregou a imagem\n" ); return -1;}
	if (!object) { printf( "nao carregou a imagem\n" ); return -1;}
	

///COMPUTE AND DISPLAY RECTIFICATION
	CvSize imageSize = {0,0};
	//cout << "aqui0" << endl;
	imageSize = cvGetSize(*s_img);
	//cout << "aqui1" << endl;
	CvMat* img1r = cvCreateMat( imageSize.height,
			imageSize.width, CV_8U );
	CvMat* img2r = cvCreateMat( imageSize.height,
...
//if useUncalibrated == 0 (BOUGUET'S METHOD) using this method
	CvMat _P1 = cvMat(3, 4, CV_64F, P1);
	CvMat _P2 = cvMat(3, 4, CV_64F, P2);
	cvStereoRectify( _pM1, _pM2, _pD1, _pD2, imageSize,
			_pR, _pT,
			&_R1, &_R2, &_P1, &_P2, 0,
			0/*CV_CALIB_ZERO_DISPARITY*/);
	//isVerticalStereo = fabs(P2[1][3]) > fabs(P2[0][3]);
	//Precompute maps for cvRemap()
	cvInitUndistortRectifyMap (_pM1, _pD1, &_R1, &_P1, *mx1, *my1);
	cvInitUndistortRectifyMap (_pM2, _pD2, &_R2, &_P2, *mx2, *my2);

	CvMat* pair = cvCreateMat( imageSize.height, imageSize.width*2,
			CV_8UC3 );

	CvMat part;
	cvRemap( image, *s_img, *mx1, *my1 );
	cvRemap( object, *s_img2, *mx2, *my2 );

	cvRemap( *s_img, img1r, *mx1, *my1 );
	cvRemap( *s_img2, img2r, *mx2, *my2 );
